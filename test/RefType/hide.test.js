...
describe('RefType.hide', (t) => {
    ...
    it('Should hide elements', (t) => {
        ...
        for (let element of reftype.elements) assert.equal(element.style.display !== 'none', true);
        reftype.hide();
        for (let element of reftype.elements) {
            assert.equal(element.style.display !== 'none', false);
            assert.equal(reftype.hidden.has(element), true);
        }
    });
